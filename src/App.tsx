...
import "./App.css";
import { CANVAS_H, CANVAS_MARING, CANVAS_W } from "./utils";
...
export default function App() {
  ...
  useEffect(() => {
    const setCursor = (x: number, y: number) => {
      if (
        Math.abs(x - editorRef.current!.width) < 5 &&
        Math.abs(y - editorRef.current!.height) < 5
      ) {
    ...
    const handleCanvasMouseDown = (e: MouseEvent) => {
      const { shiftKey } = e;
      const [x, y] = [e.offsetX - CANVAS_MARING, e.offsetY - CANVAS_MARING].map(
        (i) => i * (100 / zoomRef.current)
      );
      ...
      editorRef.current?.selectForXY(x, y, {
        shift: shiftKey,
        click: true,
        clickCount: mouseRef.current.count,
      });
      updateInputPosition();
    };
    const handleCanvasMouseMove = (e: MouseEvent) => {
      ...
      const editor = editorRef.current!;
      if (mouseRef.current.isDown && editor.hasSelection())
        editor.selectForXY(x, y, { shift: shiftKey, move: true });
    ...
    const handleCanvasWheel = (e: WheelEvent) => {
      ...
      e.preventDefault();
      if (
        zoomRef.current - e.deltaY <= 5 ||
        zoomRef.current - e.deltaY >= 12800
      ) {
        return;
      }
    ...
    <div>
      ...
      <div className="page">
        ...
        <div className="page-pannel" style={{ maxHeight: CANVAS_H }}>
          {editorRef.current && (
            <AutoResizeComp editorRef={editorRef} updateRender={updateRender} />
          )}
          {editorRef.current && <TypographyComp editorRef={editorRef} />}
          {editorRef.current && (
            <ParagraphComp editorRef={editorRef} updateRender={updateRender} />
          )}
          {editorRef.current && <FillsComp editorRef={editorRef} />}
          {editorRef.current && <OpenTypeComp editorRef={editorRef} />}
          {editorRef.current && (
            <DebugComp editorRef={editorRef} updateRender={updateRender} />
          )}
        </div>
        <div className="page-pannel" style={{ maxHeight: CANVAS_H }}>
